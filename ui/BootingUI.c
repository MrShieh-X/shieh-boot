...
EFI_STATUS addProgress(
    ...
    if (EFI_ERROR(Status)) {
        if (isPrint())Print(L"Error: Failed to Blt progress. Progress: %d Status: %d\n", Progress, Status);
        return Status;
    }
...
EFI_STATUS drawLogo(IN EFI_HANDLE ImageHandle,
    ...
    if (isPrint())Print(L"Drawing boot logo...\n");
    ...
    if ((fp) == NULL) {
        if (isPrint())Print(L"Unable to draw boot logo: Unable to get file protocol, status: %d\n", Status);
        return Status;
    }
    ...
    if (EFI_ERROR(Status)) {
        if (isPrint())Print(L"Unable to draw boot logo: Unable to execute bmp (BmpTransform)\n");
        return Status;
    }
    addProgress(gop);
